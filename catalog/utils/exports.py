...
def build_book_queryset(params, include_items=False):
    """
    Build book queryset based on filter parameters - for compatibility with base
    project.
    """
    qs = Book.objects.all()
...
def render_books_workbook(queryset, columns=None, include_items=False):
    """
    Create an Excel workbook with books data - for compatibility with base
    project.
    """
    wb = Workbook()
    ws = wb.active
